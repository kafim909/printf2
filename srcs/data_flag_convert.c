...
/*   Updated: 2021/05/25 18:30:17 by mtournay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (VF->width_size >= VF->prec && VF->period && !VF->zero
		&& !VF->minus && !VT->s && !VT->s_bol)
		if (!width_noflags_prec(var))
			return (0);
	if (VF->width_size >= VF->prec && VF->period
		&& VF->zero && !VT->s && !VT->s_bol)
...
{
	string_check(var);
	if (!VF->minus && !VF->period && !VF->prec && !VF->wd_prec
		&& !VF->wd_width && !VF->width_size && !VF->zero)
		return (1);
