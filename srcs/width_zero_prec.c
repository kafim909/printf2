...
/*   Updated: 2021/05/25 17:36:37 by mtournay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	width_zero_prec(t_var *var)
{
	...
	if (!temp)
		return (0);
	while (i < (VF->width_size - VF->prec - VT->minus_d) && !(len == (size_t)VF->width_size))
		temp[i++] = ' ';
	if (VT->minus_d)
		temp[i++] = (VU->str)[j++];
	while (i < (VF->width_size - (int)len + VT->minus_d) && !(len == (size_t)VF->width_size))
		temp[i++] = '0';
	while ((VU->str)[j])
		temp[i++] = (VU->str)[j++];
	temp[i] = '\0';
